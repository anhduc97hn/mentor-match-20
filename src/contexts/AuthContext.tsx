...
const setSession = (accessToken: string | null): void => {
  // 🚨 CRITICAL CHANGE for Next.js: Check if window is defined before using localStorage
  if (typeof window !== 'undefined') {
    if (accessToken) {
      window.localStorage.setItem("accessToken", accessToken);
      // We rely on middleware to check the cookie on the server for security
      // Axios headers are fine for subsequent client-side API calls
...
function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    // initialize();
    // 🚨 FIX: Only run this whole block on the client side
    if (typeof window !== 'undefined') {
        initialize();
    }
  }, []);
